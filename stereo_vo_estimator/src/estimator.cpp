...
//     trackFeatureBetweenFrames(key_frame, _img, key_pts_3d, cur_pts_2d);

//     // To do: undistort the points of the left image and compute relative motion with the key frame. 
//     estimateTBetweenFrames(key_pts_3d, cur_pts_2d, c_R_k, c_t_k);
//   }


//   // To do: extract new features for the current frame.
//   extractNewFeatures(_img, left_pts_2d);
//   trackFeatureLeftRight(_img, _img1, left_pts_2d, right_pts_2d);
...
//   vector<cv::Point2f> left_pts_2d_ud, right_pts_2d_ud;
//   vector<cv::Point3f> cur_pts_3d;
//   left_pts_2d_ud = undistortedPts(left_pts_2d, m_camera[0]);
//   right_pts_2d_ud = undistortedPts(right_pts_2d, m_camera[1]);
//   cur_frame.uv = left_pts_2d;
//   generate3dPoints(left_pts_2d_ud, right_pts_2d_ud,  cur_frame.xyz, cur_frame.uv);
...
//     }
//     key_frame = cur_frame;
//     rel_key_time = cur_frame.frame_time;
...
//   cv::calcOpticalFlowPyrLK(keyframe.img, cur_img, key_pts_2d, cur_pts_2d, status, err,
//                            cv::Size(40, 40), 4);
  
//   for (size_t i = 0; i < status.size(); i++) {
//     if (status[i] && !inBorder(cur_pts_2d[i],ROW,COL)) {  // Additional error threshold
//       status[i] = 0;
//     }
//   }
...
//   vector<cv::Point2f> undistorted_cur_pts = undistortedPts(cur_pts_2d, m_camera[0]);
//   vector<cv::Point2f> undistorted_key_pts = undistortedPts(key_pts_2d, m_camera[0]);

//   std::vector<uchar> mask;
  
//   //ransac
//   cv::Mat F = cv::findHomography(undistorted_key_pts, undistorted_cur_pts, cv::RANSAC, 3, mask);
//   reduceVector<cv::Point3f>(key_pts_3d, mask);
//   reduceVector<cv::Point2f>(cur_pts_2d, mask);
//   if (mask.empty()) {
//     ROS_ERROR("No inliers found after RANSAC.");
//     fail_cnt++;
//     return false;
//   }
//   if (mask.size() < 4) {
//     ROS_ERROR("Not enough inliers found after RANSAC.");
//     fail_cnt++;
//     return false;
//   }
...
//   if (key_pts_3d.size() < MIN_POINTS || cur_pts_2d.size() < MIN_POINTS) {
//     ROS_ERROR("Not enough points to estimate pose.");
//     return false;
//   }
//   vector<cv::Point2f> undistorted_pts = undistortedPts(cur_pts_2d, m_camera[0]);
//   cv::Mat rvec, tvec, inliers;
//   cv::Mat K = cv::Mat::eye(3, 3, CV_64F);
//   cv::Mat D = cv::Mat::zeros(1, 4, CV_64F);
//   bool success = cv::solvePnPRansac(key_pts_3d, undistorted_pts, K, D, rvec, tvec,
//                                 false, 200, 8.0, 0.99, inliers);
//   if (!success || inliers.rows < MIN_POINTS) {
//     return false;
//   }
//   vector<cv::Point3d> filtered_3d;
//   vector<cv::Point2d> filtered_2d;
//   for (int i = 0; i < inliers.rows; ++i) {
//   int idx = inliers.at<int>(i);
//     filtered_3d.push_back(key_pts_3d[idx]);
//     filtered_2d.push_back(undistorted_pts[idx]);
//   }
//   cv::solvePnP(filtered_3d, filtered_2d,K, D, rvec, tvec, true,
...
//   const double k = 0.04;
//   uv.clear();
//   cv::goodFeaturesToTrack(img, uv, maxCorners, qualityLevel, minDistance,
//                            cv::noArray(), blockSize, useHarrisDetector, k);

//   // if (!uv.empty()) {
//   //       cv::TermCriteria criteria = cv::TermCriteria(
//   //           cv::TermCriteria::EPS + cv::TermCriteria::COUNT, 30, 0.01);
//   //       cv::cornerSubPix(gray, uv, cv::Size(5, 5), cv::Size(-1, -1), criteria);
//   //   }
// }
...
//   cv::calcOpticalFlowPyrLK(_img, _img1, left_pts, right_pts, status, err, cv::Size(21,21), 4, criteria);
//   std::vector<cv::Point2f> tracked_left, tracked_right;
//   for (size_t i = 0; i < status.size(); i++) 
//   {
//     if (status[i] && err[i] < 50.0f) 
//     {  // Additional error threshold
//       tracked_left.push_back(left_pts[i]);
//       tracked_right.push_back(right_pts[i]);
//     }
//   }
//   // Filter out points that are too close to the image border
//   vector<cv::Point2f> filtered_left, filtered_right;
//   for (size_t i = 0; i < tracked_left.size(); i++) {
//     if (inBorder(tracked_left[i], ROW, COL) && inBorder(tracked_right[i], _img1.rows, _img1.cols)) {
//       filtered_left.push_back(tracked_left[i]);
//       filtered_right.push_back(tracked_right[i]);
//     }
//   }
//   // ransac 
//   cv::Mat mask;
//   cv::Mat F = cv::findHomography(filtered_left, filtered_right, cv::RANSAC, 3, mask);
//   vector<cv::Point2f> inlier_left, inlier_right;
//   for (size_t i = 0; i < mask.rows; i++) {
//     if (mask.empty()||mask.at<uchar>(i)) {
//       inlier_left.push_back(filtered_left[i]);
//       inlier_right.push_back(filtered_right[i]);
//     }
//   }
//   left_pts = inlier_left;
//   right_pts = inlier_right;
//   drawImage(_img, left_pts, "LK_optical_Left");
//   drawImage(_img1, right_pts, "LK_optical_Right");
//   return true;
// }
...
//   latest_pointcloud.clear();
//   latest_pointcloud = latest_frame.xyz;
//   Matrix3d w_R_b = latest_frame.w_R_c * ric[0].transpose();
//   Vector3d w_t_b = latest_frame.w_t_c + latest_frame.w_R_c * (-ric[0].transpose()*tic[0]);
//   latest_P = w_t_b;
//   latest_Q = Quaterniond(w_R_b).normalized();
//   latest_rel_P = -ric[0]*c_R_k.transpose()*c_t_k;
//   latest_rel_Q = Quaterniond(ric[0]*c_R_k.transpose()*ric[0].transpose());
// }
...
void Estimator::extractNewFeatures(const cv::Mat& img, vector<cv::Point2f>& uv) {
  ...
  cv::goodFeaturesToTrack(img, 
							...
							k);
  if(uv.empty()){
    ROS_ERROR("No features detected.");
  }
